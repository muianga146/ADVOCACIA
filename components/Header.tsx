...
const Header: React.FC<HeaderProps> = ({ onOpenContact }) => {
    ...
    const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
        ...
        if (element) {
            ...
            // Calculate absolute position + scroll Y
            const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
            // Subtract header height (approx 80px) plus some padding
    ...
    return (
        <header 
            className={`fixed top-0 w-full z-50 transition-all duration-300 border-b border-gray-200/50 dark:border-gray-800/50 ${
                isScrolled ? 'glass-header shadow-sm' : 'bg-transparent border-transparent'
            }`}
        >
                ...
                <div className="flex justify-between items-center h-20">
                    ...
                    <div className="hidden md:flex items-center">
                        <button 
                            ...
                            className="inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-sm font-medium rounded-lg text-white bg-gradient-to-r from-primary to-blue-700 hover:shadow-lg hover:shadow-primary/30 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer"
                        >
                            Consulta Gratuita
                        </button>
                    </div>
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <div className="md:hidden glass-header absolute top-20 left-0 w-full border-b border-gray-200/50 p-4 animate-fade-in-down h-screen">
